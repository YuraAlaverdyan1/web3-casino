...
test('should have insufficient funds', async ({ wallet, page }) => {
  ...
  await page.locator('*[e2e-id="/games/arcade"]').click()

  const locatorInputBet = page.locator('*[e2e-id="bet-input"]')

  await locatorInputBet.fill('10')

  // await wallet.approve()

  await expect(locatorInputBet).toHaveClass(/error-bet/)
})
