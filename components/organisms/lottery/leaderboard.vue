  ...
  <div class="mt-20 mb-20">
    <div class="nav px-[24px] md:px-[0px]">
      ...
      <div class="w-full flex items-center justify-between mt-4">
        ...
        <SelectBtn
          v-if="filteredData && +filterDataCount < filteredData.length"
    ...
    <pagination
      :filtered-data="filteredData ? filteredData : []"
  ...
  setup() {
    ...
    const filteredData = computed(() => {
      if (active.value === 1 && account.value.isConnected) {
        return onlyWin.value.filter((item) => {
          return item.user.toLowerCase() === account.value.address.toLowerCase()
        })
      } else {
        return onlyWin.value
      }
    })

    const data = computed(() => {
      return onlyWin.value && filteredData.value.slice(
